...
export default function StudentNotesPage() {
  ...
  const handleClassSelectionChange = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (selectedClassSectionId) {
      ...
      const selectedEnrollment = currentEnrollment.find(
        (enrollment: any) => enrollment.classSectionId === selectedClassSectionId
      );
      // Set the classSection object itself to the selectedClass state
      setSelectedClass(selectedEnrollment ? selectedEnrollment.classSection : null);
    } else {
      setSelectedClass(null); // Reset if "Select a Class" is chosen
    }
  }, [currentEnrollment]); // Dependency: currentEnrollment
  ...
  const openNoteInModal = useCallback((noteProps: {
    pdfUrl?: string;
    noteId?: string;
  ...
  if (!studentData) { // This case should theoretically be covered by 'error' if fetch fails, but good to keep
    return (
      <div className="flex items-center justify-center min-h-screen p-4 bg-gray-50">
        <div
    ...
    return (
      <div className="flex items-center justify-center min-h-screen p-4 bg-gray-50">
        <div
          ...
          You are not currently enrolled in any active classes or your enrollment is not active. Please contact your administrator.
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="px-4 py-6 sm:px-6 sm:py-8 bg-gray-50 min-h-screen">
      ...
      <div className="max-w-md mx-auto w-full mb-8"> {/* Added margin-bottom */}
        <label htmlFor="class-select" className="mb-1.5 block text-sm font-medium text-gray-700">Select Class</label>
        <div className="relative"> {/* Keep relative for the SVG icon positioning */}
          <select
            id="class-select"
          ...
          >
            ...
            {currentEnrollment.map((enrollment: any) => (
      ...
      <div className="max-w-screen-xl mx-auto">
        {selectedClass ? (
          ...
          <div className="text-center text-gray-600 mt-8 p-4 bg-white rounded-lg shadow-sm">
            <p className="text-lg font-medium">Please select a class from the dropdown above to view its notes.</p>
          </div>
        )}
      </div>
