...
const CreditCard: React.FC<{ title: string; balance: number; color: string }> = ({
    ...
    <div className="rounded-lg shadow-md p-6 bg-white flex flex-col items-center border-t-4" style={{ borderColor: color }}>
        ...
        <div className="max-w-4xl mx-auto py-10 px-4">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <select
                        ...
                        value={new Date(costDetails.lastUpdated).getMonth()}
                        onChange={e => {
                            const month = Number(e.target.value);
                            const year = new Date(costDetails.lastUpdated).getFullYear();
                            // Implement fetch for selected month/year here
                            // Example: fetchCostDetailsForMonthYear(month, year)
                        }}
                    >
                    ...
                    <select
                        className="border rounded px-2 py-1"
                        value={new Date(costDetails.lastUpdated).getFullYear()}
                        onChange={e => {
                            const year = Number(e.target.value);
                            const month = new Date(costDetails.lastUpdated).getMonth();
                            // Implement fetch for selected month/year here
                            // Example: fetchCostDetailsForMonthYear(month, year)
                        }}
                    >
            ...
            <div className="bg-white rounded-lg shadow p-6 flex flex-col md:flex-row justify-between items-center">
                ...
                <div className="mt-4 md:mt-0">
                    ...
                    <div className="text-md text-gray-700">{new Date(costDetails.lastUpdated).toLocaleString()}</div>
                </div>
            </div>
