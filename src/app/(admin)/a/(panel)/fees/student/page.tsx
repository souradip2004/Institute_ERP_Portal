...
const StudentFeesPage: React.FC = () => {
  ...
  const fetchAllData = async () => {
    try {
      ...
      const [studentResponse, globalFeesResponse, localFeesResponse] =
        await Promise.all([
          axios.get(`/api/students/${studentId}`),
          axios.get(
            `/api/payment/fees-collection/global-fees?institutionId=${institutionId}&studentId=${studentId}&motherClassId=${motherClassId}`
          ),
          axios.get(
            `/api/payment/fees-collection/local-fees?institutionId=${institutionId}&studentId=${studentId}`
          ),
        ]);
      ...
      const globalFeesData = Array.isArray(
        globalFeesResponse.data?.structuredResponse
      )
        ? globalFeesResponse.data.structuredResponse
        : [];
      const localFeesData = Array.isArray(
  ...
  const handlePenaltyToggle = async (
    ...
    try {
      ...
      const fee = combinedFees.find(
        (f) => f.id === feeId && f.feeType === feeType
      );
      if (!fee) {
      ...
      const response = await axios.patch(
        "/api/payment/fees-collection/penalty",
        {
      ...
      if (response.status === 200) {
        ...
        if (feeType === "global") {
          setGlobalFees((prevFees) =>
            prevFees.map((f) =>
              f.feeId === feeId ? { ...f, isPenaltyApplied: !currentStatus } : f
            )
          );
        } else {
  ...
  const handleVerifiedToggle = async (
    ...
    try {
      ...
      const response = await axios.patch(
        "/api/payment/fees-collection/verify",
        {
          feesCollectionId: fee.feesCollectionId,
          userId: adminId,
          transactions: [
      ...
      setError(
        error.response?.data?.message || "Failed to toggle verification"
      );
    } finally {
      setToggleLoading((prev) => ({ ...prev, [toggleKey]: false }));
    }
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200">
            ...
            <p className="text-sm text-gray-600 mt-1">
              Total Fees: {combinedFees.length} | Global: {globalFees.length} |
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {combinedFees.map((fee, index) => (
                  ...
                  >
                    ...
                    >
                      {fee.paymentTransactions &&
                      fee.paymentTransactions.length > 0 ? (
                        <div className="space-y-3 max-h-96 overflow-y-auto">
                          {fee.paymentTransactions.map(
                            (transaction, txIndex) => (
                              <div
                                key={transaction.id}
                              ...
                              >
                                <div className="flex items-start justify-between mb-3">
                                  ...
                                  <button
                                    ...
                                    disabled={
                                      toggleLoading[
                                        `verified-${fee.feeType}-${fee.id}-${transaction.id}`
                                      ]
                                    }
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <p className="text-2xl font-bold text-red-600 mt-2">
              {formatCurrency(
                combinedFees.reduce((sum, fee) => sum + fee.amountDue, 0)
              )}
            </p>
          </div>
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <p className="text-2xl font-bold text-green-600 mt-2">
              {formatCurrency(
                combinedFees.reduce((sum, fee) => sum + fee.amountPaid, 0)
              )}
            </p>
          </div>
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <p className="text-2xl font-bold text-yellow-600 mt-2">
              {
                combinedFees.filter((fee) => fee.paymentStatus === "PENDING")
                  .length
              }
            </p>
          </div>
