...
export default function TeacherAssignmentsPage() {
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      // Don't start fetching if there's no teacherId
      if (!teacherId) {
        return;
      }
      ...
      try {
        ...
        const endpoints = [`/api/teachers/${teacherId}/classes`];
        ...
        for (const endpoint of endpoints) {
          try {
            ...
            if (response.ok) {
              ...
              if (Array.isArray(data)) { // Handle empty array response
                fetchedClasses = data;
                break;
              }
            }
        ...
        if (fetchedClasses.length > 0) {
          ...
          // Explicitly set to an empty array if no classes were fetched
          setClasses([]);
        }
      } catch (error) {
        ...
        // This ensures the loader is always turned off after the fetch attempt
        setLoading(false);
      }
    };

    // Only call fetchClasses if teacherId is present
    if (teacherId) {
      fetchClasses();
    } else {
      // If there's no teacherId after initial check, stop loading
      setLoading(false);
    }
      ...
      return (
        <div className="text-center py-10">
          ...
          <p className="mt-1 text-sm text-gray-500">
            It seems you are not assigned to any classes at the moment.
          </p>
        </div>
      );
    }
