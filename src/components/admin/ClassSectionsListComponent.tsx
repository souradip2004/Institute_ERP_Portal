...
import { Trash2 } from 'lucide-react'; // Using lucide-react for consistency
...
export default function ClassSectionsList({ classSections, onViewClassSection, deletingClassSection, handleDeleteMotherClass }: ClassSectionsListProps) {
      ...
      {!selectedMotherClassId ? (
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                {motherClasses.map(mc => (
                  <tr key={mc.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap"><div className="text-sm font-medium text-gray-900">{mc.name}</div></td>
                    <td className="px-6 py-4 whitespace-nowrap"><div className="text-sm text-gray-900">{mc.sectionCount}</div></td>
                    <td className="px-6 py-4 whitespace-nowrap"><div className="text-sm text-gray-900">{mc.inheritedBatch ? `${mc.inheritedBatch.batchName} (${mc.inheritedBatch.year})` : 'N/A'}</div></td>
                    <td className="px-6 py-4 whitespace-nowrap"><div className="text-sm text-gray-900">{mc.inheritedSemester ? `${mc.inheritedSemester.name} (${formatDate(mc.inheritedSemester.startDate)} - ${formatDate(mc.inheritedSemester.endDate)})` : 'N/A'}</div></td>
        ...
        <>
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-gray-900">Subjects for {currentMotherClassInheritedData?.name || 'Selected Class'}</h2>
            <button onClick={() => setSelectedMotherClassId(null)} className="text-sm text-blue-600 hover:underline inline-flex items-center">
          ...
          {classSections.filter(section => section.motherClassId === selectedMotherClassId).length === 0 ? (
            <div className="text-center py-12">
              <div className="mx-auto h-12 w-12 text-gray-400"><svg className="h-12 w-12" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg></div>
                  ...
                  <tr key={section.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="h-8 w-8 flex-shrink-0 rounded-full bg-blue-100 flex items-center justify-center"><span className="text-sm font-medium text-blue-600">CS</span></div>
                        <div className="ml-4">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <button onClick={() => onViewClassSection(section.id)} className="text-purple-600 hover:text-blue-900 inline-flex items-center">
                        <svg className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                        View Details
                      </button>
                    </td>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-md mx-auto">
            ...
            <p className="mb-6 text-gray-600">Are you sure you want to delete this class and all its subjects? This action cannot be undone.</p>
