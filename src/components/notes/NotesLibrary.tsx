...
const NotesLibrary: React.FC<NotesLibraryProps> = ({
 ...
 <div className="space-y-6 p-4 sm:p-6 lg:p-8">
    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gradient-to-r from-indigo-50 to-purple-50 p-6 rounded-lg">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-2 w-full mt-4 sm:mt-0"> {/* Removed sm:w-auto from this flex container to allow it to take full width on mobile */}
        ...
        <Select>
          <SelectTrigger className="w-full sm:w-[180px] bg-white max-w-xs"> {/* Added max-w-xs for very small screens */}
    ...
    {selectedSubject && (
      <div className="bg-white p-6 rounded-lg shadow-sm border">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-5 gap-4 sm:gap-2">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto max-w-full"> {/* Added max-w-full to ensure container respects boundaries */}
            <form onSubmit={handleSearch} className="flex items-center gap-2 w-full sm:w-auto">
              ...
              <Button type="submit" variant="ghost" size="icon" className="hover:bg-gray-100 hidden sm:block">
                <Search size={18} />
              </Button>
            </form>
            <Select>
        ...
        <Tabs defaultValue="notes" className="mt-4">
          <TabsList className="flex flex-wrap gap-2"> {/* flex-wrap already present, good for mobile tabs */}
          ...
          <TabsContent value="notes" className="mt-4">
            ...
            <div className="overflow-x-auto rounded-lg border bg-white max-w-full"> {/* Added max-w-full here */}
              <Table className="w-full"> {/* Changed from min-w-[700px] to w-full */}
                <TableHeader className="bg-gray-50">
                  <TableRow>
                    ...
                    <TableHead className="py-3 w-1/4 min-w-[120px] whitespace-nowrap">Date Uploaded</TableHead> {/* Kept nowrap for dates */}
                    <TableHead className="py-3 text-right w-1/4 min-w-[140px]">Actions</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {loading ? (
                    <TableRow>
                      <TableCell colSpan={4} className="text-center py-8 px-2"> {/* Reduced padding */}
                        <div className="flex justify-center">
                          <div className="inline-block animate-spin rounded-full h-6 w-6 border-t-2 border-b-2 border-gray-900 mb-2" />
                        </div>
                    ...
                    notes
                      ...
                      .map(note => (
                        <TableRow key={note.id} className="hover:bg-gray-50">
                          ...
                          <TableCell className="font-medium pr-2 break-words max-w-[120px]">{note.title}</TableCell>
                          <TableCell className="pr-2 break-words max-w-[100px]">{note.subjectName || '-'}</TableCell>
                          <TableCell className="pr-2 whitespace-nowrap">{formatDate(note.createdAt)}</TableCell>
                          {/* Actions cell: now uses flexbox with wrapping and smaller gap */}
                          <TableCell className="text-right flex flex-wrap justify-end gap-1 px-2 py-2"> {/* Added flex-wrap, justify-end, gap-1, adjusted px/py */}
