...
const StructuredBreakdown: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const postData = async () => {
            ...
            if (localStorage.getItem("premiumTopicId") !== null) {
                url = `${process.env.NEXT_PUBLIC_BACKEND_1_SERVER_URL}/predefined/createPremiumSchedule`;
            } else {
            ...
            try {
                ...
                if (dataForBreakdown.shouldDoPostReq === true && false) {
                    ...
                    const url2 = `https://api.aiclassroom.in/api/v1/get-planner/${temp}`;
                    response = await axios.get(url2);
                    console.log('GET response:', response);
                }
                setMainSchedule(response.data);
                setWbStrId(response.data.data._id);
                localStorage.setItem("dataForBreakdown", JSON.stringify({ shouldDoPostReq: false, wbStrId: response.data.data._id }));
    ...
    const VideoIcon = () => (
        <svg width="13" height="10" viewBox="0 0 13 10" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M11.9936 1.87532C11.8696 1.861 11.7442 1.88325 11.6327 1.93932L8.71875 3.39532V5.95532L11.6327 7.41132C11.7301 7.46002 11.8384 7.48303 11.9472 7.47819C12.0561 7.47334 12.1619 7.44079 12.2546 7.38362C12.3473 7.32645 12.4239 7.24656 12.4772 7.1515C12.5304 7.05645 12.5585 6.94938 12.5588 6.84044V2.5102C12.5586 2.35355 12.501 2.20241 12.3968 2.0854C12.2926 1.9684 12.1492 1.89365 11.9936 1.87532Z" fill="#488BFF" />
            <path d="M2.31844 0.195312C1.26564 0.195312 0.398438 1.06251 0.398438 2.11531V7.23531C0.398438 8.28811 1.26564 9.15531 2.31844 9.15531H7.43844C8.49124 9.15531 9.35844 8.28811 9.35844 7.23531V2.11531C9.35844 1.06251 8.49124 0.195312 7.43844 0.195312H2.31844Z" fill="#488BFF" />
        </svg>
    );
                ...
                <div className="w-full flex flex-col gap-8">
                    <div className="w-full flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-0">
                        <div className="flex flex-row items-center">
                            <button className="p-1 hover:bg-white/20 rounded-full block lg:hidden mr-2" onClick={() => router.push('/smart-resources')}>
                                <RiArrowGoBackFill className="text-[#6B6B6B] w-5 h-5 " />
                            </button>
                            <h1 className="text-1xl lg:text-2xl font-bold">{translator("Detailed Structure Breakdown", "विस्तृत संरचना विघटन")}</h1>
                        </div>
                        <div className="flex flex-row justify-between items-center gap-4 w-full md:w-auto">
                            <div className="flex items-center gap-4">
                                <span className="text-[#6B6B6B] text-base md:text-lg">{displayedDays.length} {hasDate ? translator(" Days", " दिन") : translator("Topics", " टॉपिक्स")}</span>
                            </div>
                            <div className="bg-[#DCFCE7] text-[#166434] px-4 sm:px-6 py-1 sm:py-2 rounded-full text-sm sm:text-base font-medium">
                    ...
                    >
                        {schedule.map((dayTopics, dayIndex) => (
                            ...
                            >
                                {dayTopics.length > 0 && (
                                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mb-4 border-b-4 border-[#b4b4b4] pb-4">
                                        <div className="flex items-center gap-4">
                                            <h2 className="text-lg sm:text-xl font-medium"> {hasDate ? translator("Day", "दिन") : translator("Topic", "टॉपिक")} {dayTopics[0].dayNo}</h2>
                                            <span className="text-[#797979] text-base sm:text-lg">
                                                {hasDate ? new Date(dayTopics[0].date!).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' }) : dayIndex + 1}
                                            </span>
                                        </div>
                                    </div>
                                )}
                                    ...
                                    <div key={topic._id} className="flex flex-col gap-6 mb-6">
                                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                                            <div className="sm:ml-auto flex items-center gap-4 mt-2 sm:mt-0">
                                                <button className="flex bg-[#7763FF] hover:bg-[#6753E8] transition-colors duration-300 px-[12px] py-[6px] text-[#FFFFFF] rounded-[5px] items-center gap-2 mr-4"
                                                    onClick={() => {
                                                ...
                                                <span className="text-[#4B5563] text-sm sm:text-base">{translator("Mark as Done", "डॉन करें")}</span>
                                                <button
                                                ...
                                                >
                                                    <div className={`w-10 h-6 rounded-full transition-colors duration-300 ${topic.completed ? 'bg-blue-500' : 'bg-[#D1D5DB]'}`}></div>
                                                    <motion.div
                                            ...
                                            <div className="flex flex-col gap-4 sm:gap-8 sm:flex-row">
                                                    ...
                                                    <div className="flex flex-col gap-2">
                                                        <div className="flex items-center justify-between">
                                                            <div className="flex items-center gap-4">
                                                                ...
                                                                <span className="font-semibold text-lg sm:text-xl">{translator("Resources", "नोट्स")}</span>
                                                            </div>
                                                        </div>
                                                        <div className="flex flex-col">
                                                            <span className="text-base sm:text-lg">{translator("Study Notes", "अध्ययन नोट्स")}</span>
                                                            <span className="text-[#AFAFAF] text-sm sm:text-base">{translator("4 PDFs", "4 पीडीएफ़")}</span>
                                                        </div>
                                                    </div>
                                                    <motion.button
                                                ...
                                                >
                                                    <div className="flex flex-col gap-2">
                                                        ...
                                                        <div className="flex flex-col">
                                                            <span className="text-base sm:text-lg">{translator("Practice Quiz", "अभ्यास क्विज़")}</span>
                                                            <span className="text-[#AFAFAF] text-sm sm:text-base">{translator("AI Generated MCQs", "AI जनित बहुविकल्पीय प्रश्न")}</span>
                                                        </div>
                                                    </div>
                                                    <motion.button
                                                        onClick={() => {
