...
const ChangeSchedule = () => {
  ...
  const handleProceed = async () => {
    ...
    if (resourceId) {
      // Edit schedule via PATCH API call
      ...
      try {
        await axios.patch(`${process.env.NEXT_PUBLIC_BACKEND_1_SERVER_URL}/planner/rescheduleStudyPlan`, payload); // <-- Replace with actual endpoint
          ...
          <div className="p-4 sm:p-6">
            ...
            {/* <div>
							<label style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
								<input type="checkbox" name="whatsappNotify" checked={whatsappNotify} onChange={() => setWhatsappNotify(!whatsappNotify)} />
								{translator("Notify me on Mail", " मुझे Mail पर सूचित करें")}
							</label>
						</div> */}
