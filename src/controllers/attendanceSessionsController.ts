...
} from "@/services/attendanceSessionsService";
...
export class AttendanceSessionController {
  async createAttendanceSession(req: NextRequest) {
    try {
      ...
      return NextResponse.json(
        { error: "An error occurred while creating the attendance session" },
        { status: 500 }
      );
    }
  }
  ...
  async getAttendanceSessions(req: NextRequest) {
    try {
      ...
      if (user.role === "STUDENT" && !filters.classSectionId) {
        return NextResponse.json(
          { error: "Class section ID required for students" },
          { status: 400 }
        );
      }

      const sessions = await attendanceSessionService.getAttendanceSessions(
        filters
      );
      ...
      return NextResponse.json(
        { error: "An error occurred while fetching attendance sessions" },
        { status: 500 }
      );
    }
  }
  ...
  async getAttendanceSessionById(id: string, req?: NextRequest) {
    try {
      ...
      const session = await attendanceSessionService.getAttendanceSessionById(
        id
      );
      if (!session)
        return NextResponse.json(
      ...
      return NextResponse.json(
        { error: "An error occurred while fetching the attendance session" },
        { status: 500 }
      );
    }
  }
  ...
  async updateAttendanceSession(id: string, req: NextRequest) {
    try {
      ...
      if (user.role === "TEACHER" && user.teacher?.id !== session.teacherId) {
        return NextResponse.json(
          { error: "Teachers can only update their own sessions" },
          { status: 403 }
        );
      }
      ...
      const updatedSession =
        await attendanceSessionService.updateAttendanceSession(id, updateData);
      return NextResponse.json(updatedSession);
    } catch (error) {
      console.error(`Error updating attendance session ${id}:`, error);
      return NextResponse.json(
        { error: "An error occurred while updating the attendance session" },
        { status: 500 }
      );
    }
  }
